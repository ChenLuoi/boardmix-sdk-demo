...
export function useBoardmix() {
    const sdkBoxHelper = new (class SdkBoxHelper extends BoardMixSdk {
        ...
        protected override getFileInfo(): Promise<{
            ...
            return Promise.resolve({
                ...
                loginType: !flags.value.canEdit ? 2 : 1,
            });
        }
        ...
        shareEnable: true, // 是否支持显示分享按钮
        onShare() {
            console.log("on share");
        },
        onShareWithNode: (nodeId: string) => {
            console.log("on share node", nodeId);
        },
        async getFileUser() {
            // 获取关联用户列表，不实现则永远返回空列表

            return JSON.parse(JSON.stringify(userStore.users));
        },
    });
    ...
    const roadMapNodeGuid = ref<string>();
    ...
    function loadFile(_fileKey: string, _roadMapNodeGuid?: string) {
        fileKey.value = _fileKey;
        roadMapNodeGuid.value = _roadMapNodeGuid;
        reloadFile();
    }
    ...
    async function reloadFile() {
        ...
        sdkBoxHelper.sendMessage("START_LOADING", {
            ...
            token,
            roadMapNodeGuid: roadMapNodeGuid.value,
        });
