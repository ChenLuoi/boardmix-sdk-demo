...
export function useInjectToolbar(helper: BoardMixSdk): IFeature {
    const { multipleImportData, importData } = useOpenApi(helper);
        ...
        if (data.type === "board") {
            ...
            switch (data.key) {
                ...
                case "inject-tool-childB":
                    ...
                    helper.sendMessage("OPEN_API_CREATE_NATIVE_ELEMENT", {
                        type: "chart",
                        data: {
                            formData: {
                                itemType: 6,
